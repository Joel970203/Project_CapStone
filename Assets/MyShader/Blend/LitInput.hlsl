...
TEXTURE2D(_BlendMap); //blend맵은 base맵의 샘플러를 이용
...
inline void InitializeStandardLitSurfaceData(float2 uv, float2 uv2, out SurfaceData outSurfaceData) //add uv2
{
    half4 albedoAlpha = SampleAlbedoAlpha(uv, TEXTURE2D_ARGS(_BaseMap, sampler_BaseMap));
    outSurfaceData.alpha = Alpha(albedoAlpha.a, _BaseColor, _Cutoff);

    //
    half4 blendMapTex = SAMPLE_TEXTURE2D(_BlendMap, sampler_BaseMap, uv2);
    half4 blendColor = lerp(albedoAlpha, blendMapTex, _Blend);
    //

    half4 specGloss = SampleMetallicSpecGloss(uv, albedoAlpha.a);
    //
    outSurfaceData.albedo = blendColor.rgb * _BaseColor.rgb;
    //
